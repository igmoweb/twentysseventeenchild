...
if (have_posts()) {
	...
	while (have_posts()) {
		...
		$output .= "<div class='col-xs-6 col-sm-6 col-md-3 col-lg-3 portfolio-item'><a class='portfolio-link' href='" . get_permalink() . "' title='" . the_title_attribute('echo=0') . "'>" . "\n";

		$post_thumbnail_html = '';
		if ( has_post_thumbnail() ) {
			if ( $this->diagnostic_generate_thumbnails() )
				$this->ensure_resized_post_thumbnail( get_the_ID(), $dimensions );
			$post_thumbnail_html = get_the_post_thumbnail( null, 'portfolio-thumbnail' );
		}
		...
		if ( trim($post_thumbnail_html) != '' )
			...
			$output .= '<img class="img-centered" src="' . esc_url($thumbnails_default) . '"/>';
